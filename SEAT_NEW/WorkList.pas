...
type
  TForm3 = class(TForm)
    ...
    procedure ManGrid2GetEditorType(Sender: TObject; ACol, ARow: Integer;
      var AEditor: TEditorType);
    procedure ManGrid2SelectCell(Sender: TObject; ACol, ARow: Integer;
...
uses
 Main, WorkTong, WorkEdit , Tong_Worker;
...
begin
  ...
  Check_TabGubun;
  dateman.Date := Now;
  if Main.NotWorkdate <>'' then
  begin
    dateman.Date := StrToDate(NotWorkdate);
    INIT_MANGRID;
    chk_SAVE;
    do_GETMANDATA;
    do_GETWORKDATA;
...
begin
  LOCKFlag := 'Y';
  rect := Form3.ClientRect;
  rect.TopLeft := Form3.ClientToScreen(Rect.TopLeft);
  rect.BottomRight := Form3.ClientToScreen(Rect.BottomRight);
...
procedure TForm3.LOCK_LOGIN;
begin
   qrywork.SQL.Clear;
   qrywork.Parameters.Clear;
   qrywork.SQL.Text := 'UPDATE Seat_UserInfo set LOGINYN = :Flag where UName = :name';
   qrywork.Parameters.ParamByName('Flag').Value := 'N';
   qrywork.Parameters.ParamByName('name').Value  := MyName;
   qrywork.ExecSQL;
...
begin
  ...
  begin
    ...
    else if (Trim(ManGrid2.Cells[0, ARow]) ='기타') Or (Trim(ManGrid2.Cells[0, ARow]) ='업무대기') Or (Trim(ManGrid2.Cells[0, ARow]) ='휴가') then
    begin
       //
    end
    else
...
procedure TForm3.ManGrid2GetEditorType(Sender: TObject; ACol, ARow: Integer;
  var AEditor: TEditorType);
begin
  ...
  begin
    ...
    ManGrid2.ClearComboString;

    ManGrid2.AddComboString('휴가');
    ManGrid2.AddComboString('업무대기');
    ManGrid2.AddComboString('기타');
...
procedure TForm3.ManGrid2SelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
begin

  if (ManGrid2.Cells[0, ARow] = '휴가') Or (ManGrid2.Cells[0, ARow] = '업무대기') then
  begin

    ManGrid2.Cells[2, ARow] := '';

    if ACol = 2 then
    Begin

      ManGrid2.Options := ManGrid2.Options - [GoEditing];
...
procedure TForm3.UNLOCK_LOGIN;
begin
  qrywork.SQL.Clear;
   qrywork.Parameters.Clear;
   qrywork.SQL.Text := 'UPDATE Seat_UserInfo set LOGINYN = :Flag where UName = :name';
   qrywork.Parameters.ParamByName('Flag').Value := 'Y';
   qrywork.Parameters.ParamByName('name').Value  := MyName;
   qrywork.ExecSQL;
